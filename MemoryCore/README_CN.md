...
历史资料学习使用仓库根目录的 `scripts/team-assets.mjs`，见[资产学习说明](docs/ASSET_LEARNING_CN.md)。已移除指向不存在源码的旧 `seed-v2`、Standalone E2E 和 Skill smoke 命令；单元回归入口为 `npm run test:oss`，安装与完整链路入口见[发布验收说明](../deploy/quality-v2/ACCEPTANCE.md)。
