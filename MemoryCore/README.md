...
npm run export-tencent-vdb -- --help
```

Historical source learning uses the root `scripts/team-assets.mjs` CLI; see [asset learning](docs/ASSET_LEARNING_CN.md). Obsolete `seed-v2`, standalone E2E and Skill smoke scripts referencing absent files have been removed. Run `npm run test:oss` for unit regression and follow [release acceptance](../deploy/quality-v2/ACCEPTANCE.md) for installation and integration checks.
