...
export class TdaiGateway {
  ...
  private async ensureMetadataService(instanceId: string): Promise<MetadataService> {
    ...
    if (!svc) {
      ...
      // Background-only judges; no judge runs on the recall/injection request path.
      const { createConfiguredReviewer } = await import("../asset-quality/configured-reviewer.js");
      rawSvc.setAssetQualityReviewer({
        id: createConfiguredReviewer({ baseUrl: "", apiKey: "", model: this.config.llm.model ?? "" }).id,
        review: (...args) => {
          const qualityLlm = resolveStandaloneLlmForRuntime(this.config.llm, instanceId);
          return createConfiguredReviewer({
            baseUrl: qualityLlm.baseUrl, apiKey: qualityLlm.apiKey, model: qualityLlm.model ?? "",
            stream: this.config.llm.stream,
          }).review(...args);
        },
      });
      const { createUsageReviewer } = await import("../asset-quality/usage-reviewer.js");
      rawSvc.setAssetUsageReviewer({ id: createUsageReviewer({ baseUrl: "", apiKey: "", model: this.config.llm.model ?? "" }).id, review: (input, signal) => {
        const llm = resolveStandaloneLlmForRuntime(this.config.llm, instanceId);
        return createUsageReviewer({ baseUrl: llm.baseUrl, apiKey: llm.apiKey, model: llm.model ?? "" }).review(input, signal);
      } });
      const { createLearningGenerator } = await import("../asset-quality/learning-generator.js");
      rawSvc.setAssetLearningGenerator({ id: "asset-learning/2", generate: (input, signal) => {
        const llm = resolveStandaloneLlmForRuntime(this.config.llm, instanceId);
        return createLearningGenerator({ baseUrl: llm.baseUrl, apiKey: llm.apiKey, model: llm.model ?? "", stream: this.config.llm.stream }).generate(input, signal);
      } });
      rawSvc.quality?.start();
      rawSvc.learning?.queue.start();
  ...
  async start(): Promise<void> {
    ...
    // Resume durable review jobs after a standalone restart, without waiting for a UI request.
    if (this.config.deployMode !== "service") await this.ensureMetadataService(this.config.instanceId ?? "default");
  }
  ...
  private async doStop(): Promise<void> {
    ...
    if (this.metadataStorePool) {
      for (const svc of this.metadataServiceByInstance.values()) { svc.quality?.stop(); svc.learning?.queue.stop(); }
