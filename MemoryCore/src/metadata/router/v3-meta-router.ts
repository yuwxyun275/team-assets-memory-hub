...
import { QualityError } from "../../asset-quality/lifecycle.js";
import {
  authenticateV3,
...
const routeTable: Record<string, Handler> = {
  ...
  [`${V3_PREFIX}/asset/review`]: bind(S.assetReviewSchema, (d, c, s) =>
    s.reviewCandidateAssetForCaller(d.asset_id, d.decision, d.note, c)),
  [`${V3_PREFIX}/asset/quality/evaluate`]: bind(S.assetQualityEvaluateSchema, (d, c, s) =>
    s.evaluateAssetQualityForCaller(d, c)),
...
/** 已注册的 v3 路由路径（供测试 / 文档）。 */
for (const action of ["learning-submit", "learning-from-task", "learning-list", "learning-details", "learning-candidate", "costs", "list", "details", "submit", "decide", "retry", "policy-get", "policy-set", "audit", "rechecks", "expose", "observe", "open-exposures", "feedback", "utility", "delete-evidence", "disclosure-remember", "disclosure-list", "disclosure-read", "task-receipt", "failed-uses", "retry-use"]) {
  routeTable[`${V3_PREFIX}/asset/quality/${action}`] = bind(S.assetQualityActionSchema, (d, c, s) => s.qualityForCaller(action, d, c));
}
...
function mapErrorCode(code: string): number {
  ...
  switch (code) {
    case "quality_version_conflict":
      return 409;
    case "quality_gate_blocked":
      return 409;
    case "quality_content_budget_exceeded":
      return 413;
    case "quality_unavailable":
      return 503;
    case "quality_review_busy":
      return 429;
...
export async function handleV3MetaRoute(
  ...
  try {
    ...
    if (err instanceof ZodError) {
      sendJson(res, 400, errorEnvelope(400, "invalid_quality_request", requestId)); return true;
    }
    if (err instanceof MetadataError || err instanceof QualityError) {
  ...
  try {
    ...
    await runWithApiRequestContext(
      ...
      async () => {
        const qualityReview = pathname.startsWith(`${V3_PREFIX}/asset/quality/`);
        logMetaApiEntry(traceCtx, qualityReview ? { content: "[quality snapshot omitted]" } : body);
        ...
        logMetaApiResponse(traceCtx, qualityReview ? { ...envelope, message: "quality evaluation response", data: { content: "[quality report omitted]" } } : envelope, httpStatus);
