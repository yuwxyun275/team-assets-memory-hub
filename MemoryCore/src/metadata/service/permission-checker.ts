...
// A published team asset is a shared execution asset, not merely a document
// that teammates may look at.  Active team members therefore receive the
// default `use` capability as well as `read`; draft/candidate assets are still
// excluded by MetadataService.listAccessibleAssets before this permission
// check is reached.  Restricted/private visibility continues to require its
// stricter owner/ACL path above.
const ADMIN_ACTIONS: Permission[] = ["read", "write", "assign", "share", "use"];
const MEMBER_ACTIONS: Permission[] = ["read", "use"];
