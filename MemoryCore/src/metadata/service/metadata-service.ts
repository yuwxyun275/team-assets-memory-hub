import { AssetLearningService } from "../../asset-quality/learning-service.js";
import type { LearningGenerator } from "../../asset-quality/learning.js";
import { withModelUsage } from "../../asset-quality/model-usage.js";
/**
...
import { evaluateQuality } from "../../asset-quality/evaluator.js";
import { QualityLifecycle, exposureSchema, observationSchema, type UsageReviewer } from "../../asset-quality/lifecycle.js";
import { AssetDisclosure, disclosureScopeSchema } from "../../asset-quality/disclosure.js";
import { sceneSchema } from "../../asset-quality/scene.js";
import { snapshotSchema, type ModelReviewer, type QualitySnapshot } from "../../asset-quality/types.js";
...
export class MetadataService {
  ...
  private _assetQualityReviewer?: ModelReviewer;
  private assetQualityInFlight = 0;
  readonly quality?: QualityLifecycle;
  readonly learning?: AssetLearningService;
  setAssetLearningGenerator(generator: LearningGenerator) { if (this.learning) this.learning.queue.generator = generator; }

  setAssetQualityReviewer(reviewer: ModelReviewer): void {
    this._assetQualityReviewer = reviewer;
    if (this.quality) this.quality.reviewer = reviewer;
  }
  setAssetUsageReviewer(reviewer: UsageReviewer): void { if (this.quality) this.quality.usageReviewer = reviewer; }
  ...
  constructor(
    ...
    if (store.qualityRecords) {
      this.quality = new QualityLifecycle(store.qualityRecords, async id => await store.getAssetById(id));
      this.learning = new AssetLearningService(this, store.qualityRecords);
    }
  }
  ...
  async updateTask(taskId: string, patch: Partial<TaskEntity>): Promise<TaskEntity> {
    ...
    if (this.learning) await this.learning.recordTask(updated);
    return updated;
  }
  ...
  async createAsset(input: CreateAssetInput): Promise<AssetEntity> {
    await this.assertTeamExists(input.team_id);
    // All native callbacks/registrations pass here. Parsing success is not content approval.
    if ((input.visibility ?? "team") !== "private" && input.status === "approved") input = { ...input, status: "candidate" };
  ...
  async updateAsset(assetId: string, patch: Partial<AssetEntity>): Promise<AssetEntity> {
    const old = await this.getAssetById(assetId);
    if (!old) throw new MetadataError("asset_not_found", `asset not found: ${assetId}`);
    const changedContent = ["name", "description", "content_ref", "source_ref", "metadata_json", "version"].some(k =>
      k in patch && (patch as any)[k] !== (old as any)[k]);
    if (changedContent) patch = { ...patch, version: old.version + 1 };
    if ((patch.visibility ?? old.visibility) !== "private") {
      if (changedContent) patch = { ...patch, status: "candidate" };
      else if (patch.status === "approved" && !await this.quality?.publication(old.team_id, assetId)) {
        throw new MetadataError("quality_gate_blocked", "团队发布必须先完成内容评估并由负责人确认");
      }
      else if (patch.visibility && patch.visibility !== old.visibility && !await this.quality?.publication(old.team_id, assetId)) patch = { ...patch, status: "candidate" };
    }
  ...
  async ensureChatMemoryAsset(params: {
    ...
    if (this.ensuredChatMemoryAssets.has(assetId)) {
      ...
      if (cached) return this.publishLegacyRuntimeAsset(cached);
      // 缓存脏了（被外部删除）—— 清掉重来
    ...
    // 兼容旧版本错误写入的 status=active。active 从未属于 AssetStatus，
    // 会被 list-accessible(action=use) 的 approved 门槛永久过滤。
    asset = await this.publishLegacyRuntimeAsset(asset);
  ...
  /**
   * v2.0.0 早期版本把自动生成的 skill/chat_memory 写成了 status=active。
   * 该值不在 AssetStatus 契约中，也无法通过 use 的 approved 发布门槛。
   * ensure 是这些运行时资产的权威创建入口，因此在幂等回读时顺手自愈；
   * draft/candidate 等真实治理状态仍保持不变。
   */
  private async publishLegacyRuntimeAsset(asset: AssetEntity): Promise<AssetEntity> {
    if (
      (asset.asset_type === "skill" || asset.asset_type === "chat_memory")
      && String(asset.status) === "active"
    ) {
      return this.updateAsset(asset.asset_id, { status: asset.visibility === "private" ? "approved" : "candidate" });
    }
    return asset;
  }
  ...
  async listAgentFixedAssetsWithDetail(
    ...
    for (const b of bindingPage.items) {
      ...
      if (asset.visibility !== "private" && !await this.quality?.publication(asset.team_id, asset.asset_id)) continue;
  ...
  // 权限判定（懒加载 ACL）
  // ============================================================
  async checkAssetPermission(params: CheckPermissionParams, dependencyPath = new Set<string>()): Promise<PermCheckResult> {
    ...
    if (dependencyPath.has(asset.asset_id) || dependencyPath.size >= 16) return { allowed: false, reason: "source_dependency_invalid" };
    if (asset.source_type === "asset_learning") {
      let sources: Array<{ asset_id?: string }>;
      try { sources = JSON.parse(asset.metadata_json || "{}").learning?.source_manifest ?? []; }
      catch { return { allowed: false, reason: "source_dependency_invalid" }; }
      if (!Array.isArray(sources) || sources.length > 24) return { allowed: false, reason: "source_dependency_invalid" };
      const nextPath = new Set(dependencyPath).add(asset.asset_id);
      for (const source of sources) if (source.asset_id) {
        const origin = await this.getAssetById(source.asset_id);
        if (!origin || origin.team_id !== asset.team_id || !(await this.checkAssetPermission({
          user_id: userId, asset_id: source.asset_id, action: "read", agent_id: params.agent_id,
        }, nextPath)).allowed) return { allowed: false, reason: "source_permission_revoked" };
      }
    }
    if (params.action === "use" && asset.visibility !== "private" && !await this.quality?.publication(asset.team_id, asset.asset_id)) {
      return { allowed: false, reason: "quality_not_published" };
    }
    ...
    // Restricted assets deliberately skip role defaults, so an empty ACL is
    // inconclusive even for a permission normally granted to team members.
    const needsRestrictedAcl = asset.visibility === "restricted" && membership?.status === "active";
    ...
    for (const teamId of teamIds) {
      ...
      while (true) {
        ...
        for (const asset of page.items) {
          ...
          if (FILTERED_STATUSES.includes(asset.status)) continue;
          // “可阅读”和“可作为团队权威上下文使用”是两种不同权限语义。
          // candidate/draft 仍需要在 Hub 审核页可见，但在发布为 approved
          // 之前绝不能被 Proxy 注入到 Coding Agent。
          if (action === "use" && asset.status !== "approved") continue;
          ...
          if (perm.allowed) {
            ...
            const publication = action === "use" ? await this.quality?.publication(asset.team_id, asset.asset_id) : null;
            result.push(publication ? { ...asset, quality_publication: {
              ...publication, snapshot: { ...publication.snapshot, sources: [] },
              report: { report_id: publication.report.report_id, snapshot_sha256: publication.report.snapshot_sha256,
                decision: publication.report.decision, scorecard: publication.report.scorecard },
            } } as AssetEntity : asset);
          }
        }
        if (offset + page.items.length >= page.total) break;
        offset += limit;
  ...
  /**
   * Task governance belongs to the team, not only to the person who created
   * the row.  The creator may manage it directly; an active team admin may do
   * the same for automation such as evidence receipts and CI completion.
   * Ordinary members remain read/participation-only.
   */
  private async assertCallerIsTaskCreatorOrTeamAdmin(
    ctx: V3AuthContext,
    taskId: string,
  ): Promise<TaskEntity> {
    const task = await this.getTaskById(taskId);
    if (!task) throw new MetadataError("task_not_found", `task not found: ${taskId}`);
    const callerId = this.requireCallerId(ctx);
    if (callerId === task.creator_user_id) return task;
    await this.assertCallerIsTeamAdmin(ctx, task.team_id);
    return task;
  }
  ...
  private async assertCallerIsAssetOwnerOrTeamAdmin(ctx: V3AuthContext, assetId: string): Promise<AssetEntity> {
    ...
    if (asset.visibility === "private") throw new MetadataError("permission_denied", "私有资产仅所有者可访问");
  ...
  async updateTaskForCaller(
    ...
    await this.assertCallerIsTaskCreatorOrTeamAdmin(ctx, taskId);
  ...
  /** Explicit pre-publication content review. Never called by retrieval or publication. */
  async getAssetForCaller(assetId: string, ctx: V3AuthContext) {
    const actor = this.requireCallerId(ctx);
    const asset = await this.getAssetById(assetId);
    if (!asset) throw new MetadataError("asset_not_found", "资产不存在");
    await this.requireActiveTeamMember(ctx, asset.team_id);
    if (!(await this.checkAssetPermission({ user_id: actor, asset_id: assetId, action: "read" })).allowed) throw new MetadataError("permission_denied", "无权读取资产");
    return asset;
  }

  async listAssetsForCaller(input: any, ctx: V3AuthContext) {
    const actor = this.requireCallerId(ctx);
    await this.requireActiveTeamMember(ctx, input.team_id);
    return this.listAccessibleAssets({ ...input, user_id: actor, user_key: undefined, action: "read" });
  }

  async listAccessibleAssetsForCaller(input: any, ctx: V3AuthContext) {
    const actor = this.requireCallerId(ctx);
    if (input.user_id && input.user_id !== actor) throw new MetadataError("permission_denied", "不能代替其他用户查询资产权限");
    return this.listAccessibleAssets({ ...input, user_id: actor, user_key: undefined });
  }

  async qualityForCaller(action: string, input: Record<string, any>, ctx: V3AuthContext): Promise<any> {
    const quality = this.quality;
    if (!quality) throw new MetadataError("quality_unavailable", "当前存储不支持质量工作流");
    const team = String(input.team_id ?? "");
    const member = await this.requireActiveTeamMember(ctx, team);
    const actor = this.requireCallerId(ctx);
    const manager = member.role === "admin" || member.role === "reviewer";
    const canManage = () => { if (!manager) throw new MetadataError("permission_denied", "需要团队负责人或审核者权限"); };
    const readable = async (assetId: string, write = false) => {
      const asset = await this.getAssetById(assetId);
      if (!asset || asset.team_id !== team) throw new MetadataError("asset_not_found", "当前团队内找不到该资产");
      const permission = await this.checkAssetPermission({ user_id: actor, asset_id: assetId, action: "read" });
      if (!permission.allowed || (write && asset.owner_user_id !== actor && !manager)) throw new MetadataError("permission_denied", "无权访问该资产或管理评估材料");
      return asset;
    };
    if (action.startsWith("learning-") || action === "costs") return this.learning!.action(action, team, actor, input);
    if (["disclosure-remember", "disclosure-list", "disclosure-read"].includes(action)) {
      const { team_id: _team, ...payload } = input;
      const scope = disclosureScopeSchema.parse(payload);
      const agent = await this.getAgentById(scope.agent_id);
      if (!agent || agent.team_id !== team || agent.status !== "active"
          || (agent.owner_user_id !== actor && agent.visibility !== "team")) {
        throw new MetadataError("permission_denied", "必须使用自己或当前团队共享的有效 Agent");
      }
      if (scope.task_id) {
        const task = await this.getTaskById(scope.task_id);
        if (!task || task.team_id !== team) throw new MetadataError("permission_denied", "任务不属于当前团队");
      }
      const disclosure = new AssetDisclosure(quality, async assetId => {
        const asset = await this.getAssetById(assetId);
        if (!asset || asset.team_id !== team || asset.status !== "approved") return null;
        const permission = await this.checkAssetPermission({ user_id: actor, agent_id: scope.agent_id, asset_id: assetId, action: "use" });
        return permission.allowed ? quality.publication(team, assetId) : null;
      });
      if (action === "disclosure-remember") return disclosure.remember(team, actor, payload);
      if (action === "disclosure-list") return disclosure.list(team, actor, payload);
      return disclosure.read(team, actor, payload);
    }
    if (action === "policy-get") return quality.policy(team);
    if (action === "policy-set") { canManage(); return quality.setPolicy(team, actor, input.policy); }
    if (action === "list") {
      const page = await this.store.listAssetsByTeam(team, { limit: Math.min(Number(input.limit) || 100, 200), offset: Math.max(0, Number(input.offset) || 0) });
      const allowed = await Promise.all(page.items.map(a => this.checkAssetPermission({ user_id: actor, asset_id: a.asset_id, action: "read" })));
      const items = page.items.filter((_, i) => allowed[i].allowed);
      return { items, total: page.total };
    }
    if (action === "audit" || action === "rechecks") {
      canManage();
      const rows = await quality.records.list(action === "audit" ? "audit" : "recheck", team, String(input.after ?? ""), 100);
      const items = [];
      for (const row of rows) {
        let assetId = row.data.asset_id;
        if (!assetId && row.data.target) {
          const revision = await quality.records.get(`revision:${row.data.target}`);
          const exposure = !revision ? await quality.records.get(row.data.target) : null;
          assetId = revision?.data.asset_id ?? exposure?.data.asset_id;
        }
        if (assetId && !(await this.checkAssetPermission({ user_id: actor, asset_id: assetId, action: "read" })).allowed) continue;
        if (!assetId && row.data.target !== team) continue;
        items.push(row);
      }
      return { items, next_cursor: rows.length === 100 ? rows.at(-1)!.key : null };
    }
    if (action === "details") {
      await readable(input.asset_id);
      const result = await quality.details(team, input.asset_id);
      if (!manager) { result.exposures = result.exposures.filter(e => e.data.actor === actor); result.utility = quality.utility(result.exposures); }
      return { ...result, contextual_utility: quality.contextualUtilities(result.exposures) };
    }
    if (action === "task-receipt" || action === "failed-uses") {
      if (action === "failed-uses") canManage();
      if (action === "task-receipt") {
        if (typeof input.task_id !== "string" || !input.task_id || input.task_id.length > 200) throw new MetadataError("invalid_param", "任务 ID 必填");
        const task = await this.getTaskById(input.task_id);
        if (!task || task.team_id !== team) throw new MetadataError("permission_denied", "任务不属于当前团队");
      }
      const records = await quality.usageRecords(team, action === "task-receipt" ? input.task_id : undefined);
      const permitted = [], names = new Map<string, string>();
      for (const record of records) {
        if (!manager && record.data.actor !== actor) continue;
        if (action === "failed-uses" && record.data.state !== "failed") continue;
        const asset = await this.getAssetById(record.data.asset_id);
        if (!asset || asset.team_id !== team) continue;
        if (!(await this.checkAssetPermission({ user_id: actor, asset_id: asset.asset_id, action: "read" })).allowed) continue;
        permitted.push(record); names.set(asset.asset_id, asset.name);
      }
      // Read-through join of the atomically saved effect receipts. No task metadata dual-write race,
      // no mutation of native used/validated/contributed counters from a model's helpful verdict.
      const receipt = quality.taskReceipt(permitted);
      const feedbackRows = new Map<string, Promise<Awaited<ReturnType<typeof quality.details>>>>();
      return { ...receipt, items: await Promise.all(receipt.items.map(async r => {
        let sceneFeedback = null;
        if (r.recommendation_scene) {
          if (!feedbackRows.has(r.asset_id)) feedbackRows.set(r.asset_id, quality.details(team, r.asset_id));
          const detail = await feedbackRows.get(r.asset_id)!;
          // Aggregate only. Do not disclose another member's raw conversations.
          const { hash: _hash, ...before } = r.recommendation_scene;
          sceneFeedback = quality.utility(detail.exposures, r.context, r.revision_id, before);
        }
        return { ...r, asset_name: names.get(r.asset_id) ?? r.asset_id, scene_feedback: sceneFeedback };
      })) };
    }
    if (action === "submit") {
      const parsed = snapshotSchema.parse(input.snapshot);
      const asset = await readable(parsed.asset_id, true);
      if (asset.asset_type !== parsed.asset_type || asset.version !== input.expected_asset_version) throw new MetadataError("quality_version_conflict", "类型或版本不一致");
      return quality.submit(team, actor, asset.version, parsed);
    }
    if (["decide", "retry"].includes(action)) {
      const revision = await quality.records.get(`revision:${String(input.revision_id)}`);
      if (!revision || revision.team !== team) throw new MetadataError("quality_not_found", "当前团队找不到该评估版本");
      const asset = await readable(revision.data.asset_id, true);
      if (action === "retry") return quality.retry(team, actor, input.revision_id);
      canManage();
      if (!["approve", "reject", "suspend"].includes(input.decision) || typeof input.note !== "string" || !input.note.trim() || input.note.length > 2000) throw new MetadataError("invalid_param", "审核决定和说明必填");
      const result = await quality.decide(team, actor, input.revision_id, input.decision, input.note);
      if (input.decision === "approve") await this.updateAsset(asset.asset_id, { status: "approved" });
      return result;
    }
    if (action === "expose") {
      const data = exposureSchema.parse(input.exposure);
      const asset = await readable(data.asset_id);
      const task = await this.getTaskById(data.task_id);
      if (!task || task.team_id !== team) throw new MetadataError("permission_denied", "观察必须关联当前团队的真实任务");
      const allowed = await this.checkAssetPermission({ user_id: actor, asset_id: asset.asset_id, action: "use" });
      if (!allowed.allowed) throw new MetadataError("permission_denied", "资产不可用于当前任务");
      return quality.expose(team, actor, data);
    }
    if (action === "observe") return quality.observe(team, actor, observationSchema.parse(input.observation));
    if (action === "open-exposures") {
      const records = [];
      let after = "";
      for (;;) {
        const page = await quality.records.list("exposure", team, after, 200);
        records.push(...page.filter(r => r.data.actor === actor && r.data.session_id === input.session_id && r.data.task_id === input.task_id && r.data.expires > Date.now() && !r.data.window_closed));
        if (page.length < 200) break;
        after = page[page.length - 1].key;
      }
      return { items: records.sort((a, b) => b.data.created - a.data.created).slice(0, 12).map(r => ({
        key: r.key, baseline_event_ids: r.data.baseline_event_ids ?? [], event_ids: r.data.events.map((e: any) => e.id),
      })) };
    }
    if (action === "feedback") {
      const record = await quality.records.get(String(input.exposure_id));
      if (!record || record.team !== team || (record.data.actor !== actor && !manager)) throw new MetadataError("permission_denied", "无权反馈该观察记录");
      await readable(record.data.asset_id);
      return quality.feedback(team, actor, input.exposure_id, input.outcome, input.note);
    }
    if (action === "retry-use") {
      const record = await quality.records.get(String(input.exposure_id));
      if (!record || record.team !== team || record.kind !== "exposure" || (record.data.actor !== actor && !manager)) throw new MetadataError("permission_denied", "无权重试该观察记录");
      await readable(record.data.asset_id);
      return quality.retryUsage(team, actor, input.exposure_id, input.note);
    }
    if (action === "utility") {
      const data = exposureSchema.shape.context.parse(input.context);
      const asset = await readable(input.asset_id);
      const detail = await quality.details(team, asset.asset_id);
      return quality.utility(detail.exposures, data, String(input.revision_id), input.before === undefined ? undefined : sceneSchema.parse(input.before));
    }
    if (action === "delete-evidence") {
      canManage();
      const record = await quality.records.get(String(input.exposure_id));
      if (!record || record.team !== team || record.kind !== "exposure") throw new MetadataError("quality_not_found", "观察不存在");
      await readable(record.data.asset_id);
      if (!await quality.records.delete(record.key, record.rev)) throw new MetadataError("quality_version_conflict", "记录已更新");
      return { deleted: true, effects_removed: true };
    }
    throw new MetadataError("quality_not_found", "未知质量操作");
  }

  async evaluateAssetQualityForCaller(
    input: { snapshot: QualitySnapshot; expected_asset_version?: number },
    ctx: V3AuthContext,
  ) {
    const parsed = snapshotSchema.safeParse(input.snapshot);
    if (!parsed.success) throw new MetadataError("invalid_quality_snapshot", "invalid or oversized content snapshot");
    const snapshot = parsed.data;
    const asset = await this.assertCallerIsAssetOwnerOrTeamAdmin(ctx, snapshot.asset_id);
    await this.requireActiveTeamMember(ctx, asset.team_id);
    if (asset.asset_type !== snapshot.asset_type) {
      throw new MetadataError("invalid_quality_snapshot", "snapshot type differs from registered asset type");
    }
    if (input.expected_asset_version !== undefined && input.expected_asset_version !== asset.version) {
      throw new MetadataError("quality_version_conflict", "registered asset version changed; reload before review");
    }
    if (this.assetQualityInFlight >= 2) throw new MetadataError("quality_review_busy", "at most two concurrent reviews per instance");
    this.assetQualityInFlight++;
    try {
      const report = this.quality ? await withModelUsage({ records: this.quality.records, team: asset.team_id, actor: this.requireCallerId(ctx), purpose: "quality_review", asset_id: asset.asset_id }, () => evaluateQuality(snapshot, { reviewer: this._assetQualityReviewer })) : await evaluateQuality(snapshot, { reviewer: this._assetQualityReviewer });
      return { ...report, registration: { team_id: asset.team_id, asset_version: asset.version, content_binding: "caller_supplied_unverified" as const } };
    } finally {
      this.assetQualityInFlight--;
    }
  }
   ...
   * 审核任务回流产生的候选资产。
   *
   * 普通 asset/update 仍坚持 owner-only；候选审核则允许团队 admin/reviewer
   * 执行，并由服务端写入审核人、时间和决定，防止前端伪造审计字段。
   */
  async reviewCandidateAssetForCaller(
    assetId: string,
    decision: "approve" | "reject",
    note: string | undefined,
    ctx: V3AuthContext,
  ): Promise<AssetEntity> {
    if (decision === "approve") throw new MetadataError("quality_gate_blocked", "请在质量中心核对指定内容版本和报告后发布");
    const asset = await this.getAssetById(assetId);
    if (!asset) throw new MetadataError("asset_not_found", `asset not found: ${assetId}`);
    if (asset.status !== "candidate") {
      throw new MetadataError("invalid_status", `only candidate assets can be reviewed: ${asset.status}`);
    }
    const member = await this.requireActiveTeamMember(ctx, asset.team_id);
    const callerId = this.requireCallerId(ctx);
    const mayReview = member.role === "admin" || member.role === "reviewer" || asset.owner_user_id === callerId;
    if (!mayReview) throw new MetadataError("permission_denied", "candidate review requires admin/reviewer role");
    ...
    if (metadata.team_asset_candidate && typeof metadata.team_asset_candidate === "object") {
      metadata.team_asset_candidate.review_required = false;
      metadata.team_asset_candidate.review_status = "rejected";
    }
    const bench = metadata.team_asset_bench;
    if (bench && typeof bench === "object") {
      bench.updated_at = reviewedAt;
      bench.evidence_state = "corrected";
      if (bench.asset_payload && typeof bench.asset_payload === "object") {
        bench.asset_payload.updated_at = reviewedAt;
        bench.asset_payload.evidence_state = "corrected";
      }
    }
