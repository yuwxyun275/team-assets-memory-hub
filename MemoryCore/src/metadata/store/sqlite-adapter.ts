...
export class SqliteMetadataStore implements IMetadataStore {
  ...
  readonly qualityRecords: import("../../asset-quality/records.js").QualityRecords = {
    get: (key) => {
      const row = this.db.prepare("SELECT payload FROM meta_quality_records WHERE key=?").get(key);
      return row ? JSON.parse(String(row.payload)) : null;
    },
    list: (kind, team, after = "", limit = 100) => this.db.prepare(
      "SELECT payload FROM meta_quality_records WHERE kind=? AND (? IS NULL OR team=?) AND key>? ORDER BY key LIMIT ?"
    ).all(kind, team ?? null, team ?? null, after, Math.min(limit, 500)).map(row => JSON.parse(String(row.payload))),
    cas: (record, expected) => {
      const next = { ...record, rev: expected + 1, updated: Date.now() };
      return expected === 0
        ? this.db.prepare("INSERT OR IGNORE INTO meta_quality_records(key,team,kind,rev,payload) VALUES(?,?,?,?,?)")
          .run(record.key, record.team, record.kind, 1, JSON.stringify(next)).changes === 1
        : this.db.prepare("UPDATE meta_quality_records SET rev=?,payload=? WHERE key=? AND rev=?")
          .run(next.rev, JSON.stringify(next), record.key, expected).changes === 1;
    },
    delete: (key, expected) => this.db.prepare("DELETE FROM meta_quality_records WHERE key=? AND rev=?").run(key, expected).changes === 1,
  };
  ...
  private createSchema(): void {
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS meta_quality_records (
        key TEXT PRIMARY KEY, team TEXT NOT NULL, kind TEXT NOT NULL, rev INTEGER NOT NULL, payload TEXT NOT NULL
      );
      CREATE INDEX IF NOT EXISTS idx_meta_quality_team_kind ON meta_quality_records(kind,team,key);
