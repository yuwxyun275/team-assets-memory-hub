...
export class MongoMetadataStore implements IMetadataStore {
  readonly qualityRecords: import("../../asset-quality/records.js").QualityRecords = {
    get: async key => {
      const row = await this.db.collection("meta_quality_records").findOne({ key }, PROJECT_NO_ID);
      return row as import("../../asset-quality/records.js").QualityRecord | null;
    },
    list: async (kind, team, after = "", limit = 100) => await this.db.collection("meta_quality_records")
      .find({ kind, ...(team ? { team } : {}), key: { $gt: after } }, PROJECT_NO_ID).sort({ key: 1 }).limit(Math.min(limit, 500)).toArray() as import("../../asset-quality/records.js").QualityRecord[],
    cas: async (record, expected) => {
      const next = { ...record, rev: expected + 1, updated: Date.now() };
      if (expected > 0) return (await this.db.collection("meta_quality_records").replaceOne({ key: record.key, rev: expected }, next)).modifiedCount === 1;
      try { await this.db.collection("meta_quality_records").insertOne(next); return true; }
      catch (e) { if ((e as { code?: number }).code === 11000) return false; throw e; }
    },
    delete: async (key, rev) => (await this.db.collection("meta_quality_records").deleteOne({ key, rev })).deletedCount === 1,
  };
  ...
  async init(): Promise<void> {
    await this.db.collection("meta_quality_records").createIndex({ key: 1 }, { unique: true });
    await this.db.collection("meta_quality_records").createIndex({ kind: 1, team: 1, key: 1 });
    // ── meta_users ──
