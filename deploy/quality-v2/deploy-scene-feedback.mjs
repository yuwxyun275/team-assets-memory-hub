/** Local scene-feedback overlay. Core/Hub/Proxy have rollback containers.
 * The ranker uses the repository's read-only bind mount and is restarted separately.
 * Does not alter model configuration or historical experiments. */
...
if (selected && !['core', 'hub', 'proxy'].includes(selected)) throw new Error('component must be core, hub or proxy');
const plans = [
  { name: 'tdai-memory-core', part: 'core', files: [
    ...['asset-quality/scene.ts', 'asset-quality/disclosure.ts', 'asset-quality/lifecycle.ts', 'asset-quality/usage-result.ts', 'asset-quality/usage-reviewer.ts',
  ...
  { name: 'tdai-proxy', part: 'proxy', files: [
    ...['injection/injectors/quality-observer.ts', 'injection/injectors/team-assets-orchestrator-injector.ts'].map(file => [`MemoryProxy/src/${file}`, `/app/src/${file}`]),
  ] },
...
if (!process.argv.includes('--apply')) {
  ...
  const backup = join(root, 'output/quality-deployment', `scene-feedback-${stamp}`);
  mkdirSync(backup, { recursive: true, mode: 0o700 });
  for (const p of plans) {
    p.info = JSON.parse(docker('inspect', p.name))[0];
    p.previous = `${p.name}-before-scene-feedback-${stamp}`;
    p.image = `tdai-local/memory-${p.part}:scene-feedback-${stamp}`;
  }
  ...
  for (const p of plans) {
    const build = `scene-feedback-build-${p.part}-${Date.now()}`;
