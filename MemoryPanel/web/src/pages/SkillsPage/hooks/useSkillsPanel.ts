...
export function useSkillsPanel() {
  ...
  const skillsWithCache = useMemo(
    () => skills.map((s) => isLearnedSkill(s) ? s : applyCachedDetail(s)),
    [skills, cacheVersion],
  );
  ...
  // 详情只在用户点开时按需加载。
  useEffect(() => {
    const selected = skills.find(s => s.skill_id === selectedSkillId);
    if (selected && !isLearnedSkill(selected)) void preloadSkillDetail(selected.skill_id);
  }, [selectedSkillId, skills, preloadSkillDetail]);
  ...
  const handleDelete = useCallback(
    async (skill: SkillSummary) => {
      ...
      if (isLearnedSkill(skill)) {
        tea.notify.warning('提炼的 Skill 请在资产质量中心管理审核和发布版本。');
        return;
      }
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      if (isLearnedSkill(selectedSkill)) {
        if (!activeTeamId) return;
        const view = await getLearnedSkill(activeTeamId, exportSkillId);
        const url = URL.createObjectURL(new Blob([view.snapshot.body], { type: 'text/markdown;charset=utf-8' }));
        const a = document.createElement('a');
        a.href = url;
        a.download = view.published ? 'SKILL.md' : 'candidate-SKILL.md';
  ...
  }, [selectedSkillId, selectedSkill, activeTeamId]);
