...
import LearnedSkillDetailPane from './LearnedSkillDetailPane';
import { isLearnedSkill } from '@/lib/learned-skill';
...
export default function SkillsPanel({
  ...
  const learnedSelected = isLearnedSkill(selectedSkill);
    ...
    <div className="_memory-skills-body">
      ...
      <AssetPageHeader
        ...
        actions={
          <>
            {(() => {
              if (learnedSelected) return <Button onClick={() => navigate(`/quality?asset_id=${encodeURIComponent(selectedSkillId!)}`)}>审核与版本管理</Button>;
            ...
            {(() => {
              ...
              return (
                ...
                >
                  {learnedSelected ? '导出正文（MD）' : t('skills.export')}
                </Button>
              );
            })()}
            <Button
      ...
      <AssetSplitLayout
        ...
        sidebar={
          <AssetListPanel
            ...
            renderItem={(s) => {
              ...
              const canManage = ownerIsMe && !isLearnedSkill(s);
              ...
              return (
                <>
                  ...
                  <AssetItemBadges>
                    {isLearnedSkill(s) && <span className="_learned-skill-badge">资料提炼</span>}
                    {s.owner_user_id && (
                      <UserBadge
        ...
        detail={
          learnedSelected && selectedSkill && activeTeamId ? <LearnedSkillDetailPane key={`${activeTeamId}:${selectedSkill.skill_id}`} teamId={activeTeamId} skillId={selectedSkill.skill_id} name={selectedSkill.name} /> :
          selectedSkillId && !selectedSkill ? <p role="status">正在读取 Skill 信息…</p> :
          <SkillDetailPane
