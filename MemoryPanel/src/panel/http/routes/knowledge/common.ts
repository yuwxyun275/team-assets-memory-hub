...
export async function ensureKnowledgeAsset(
  ...
  params: {
    ...
    /** 构建中保持 draft；KS 确认 ready 后发布为 approved。 */
  ...
  if (getEnv.code === 0 && getEnv.data) {
    const existing = getEnv.data as { status?: unknown };
    if (params.status === 'approved' && existing.status === 'draft') {
      const updateEnv = await deps.metaKernel.invoke(
        'asset/update',
        { asset_id: params.assetId, status: 'approved' },
        ctx,
      );
      if (updateEnv.code !== 0) {
        log.error('[ensure-knowledge-asset] publish rejected', {
          asset_id: params.assetId, code: updateEnv.code, message: updateEnv.message,
        });
        return { ok: false, env: updateEnv };
      }
      log.info('[ensure-knowledge-asset] published', {
        asset_id: params.assetId, status: 'approved',
      });
    }
