...
export function registerKnowledgeWikiRoutes(api: Hono, deps: PanelDeps): void {
  ...
  api.post('/knowledge/wiki/create', mw, async (c) => {
    ...
    // 创建时以 draft 注册，前端可立刻看到抽取中的 wiki；ready callback 再发布。
  ...
  api.post('/knowledge/wiki/get', mw, async (c) => {
    ...
    return runKs(c, async () => {
      ...
      if (status === 'ready') {
        const ready = detail as typeof detail & {
          team_id: string;
          name: string;
          owner_user_id: string;
          service_url?: string | null;
        };
        // callback 进程重启导致内存任务丢失时，由 owner 的轮询请求幂等兜底发布。
        if (ready.owner_user_id === gate.userId) {
          const reg = await ensureKnowledgeAsset(deps, ctx, {
            assetId: wikiId,
            teamId: ready.team_id,
            assetType: ASSET_TYPE_WIKI,
            name: ready.name,
            ownerUserId: ready.owner_user_id,
            serviceUrl: ready.service_url,
            status: 'approved',
          });
          if (!reg.ok) {
            throw new DomainError(reg.env.message, 'KNOWLEDGE_ASSET_PUBLISH_FAILED', 502);
          }
        }
      }
      const stored =
