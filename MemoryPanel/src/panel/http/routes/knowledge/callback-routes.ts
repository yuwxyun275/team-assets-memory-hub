 ...
 *     这是 Proxy 注入的唯一闸门。
 *   - wiki/code-graph ready 时以 owner 身份发布 meta_asset；
...
/**
 * knowledge ready 后用内存任务表里 stash 的 owner key 发布 meta asset。
      ...
      try {
        ...
        if (body.type === 'wiki') {
          ...
          if (!detail?.service_url) {
            ...
            await publishKnowledgeAsset(deps, log, body.knowledge_id, {
