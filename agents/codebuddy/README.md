...
## CodeBuddy CLI 团队资产接入

CLI 使用 `node agents/codebuddy/cli.mjs` 显式绑定团队、Agent、任务与工作区；配置和验收说明见 [资产学习与 CLI 文档](../../MemoryCore/docs/ASSET_LEARNING_CN.md)。`node scripts/verify-codebuddy-cli.mjs` 使用已安装 CLI 和本地模拟模型验证协议，不调用真实模型。

下面的 models.json 与交互表单说明主要描述原有 IDE／桌面端入口，不能替代 CLI 验收。
