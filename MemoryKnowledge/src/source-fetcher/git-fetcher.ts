...
function localHttpEnabledFromEnv(): boolean {
  const raw = process.env.KNOWLEDGE_ALLOW_LOCAL_HTTP;
  return raw != null && ["1", "true", "yes", "on"].includes(raw.trim().toLowerCase());
}

const LOCAL_DEMO_HOSTS = new Set(["localhost", "127.0.0.1", "::1", "host.docker.internal"]);
...
export interface GitSourceFetcherOptions {
  ...
  ssrfCheck?: boolean;
  /**
   * 只用于本机离线演示：允许从明确的环回/host.docker.internal HTTP 地址拉取。
   * 默认关闭；即使开启也不会放行任意私网或公网明文 HTTP。
   */
...
export class GitSourceFetcher implements ISourceFetcher {
  ...
  constructor(opts?: GitSourceFetcherOptions) {
    ...
    this.allowLocalHttp = opts?.allowLocalHttp ?? localHttpEnabledFromEnv();
  }
  ...
  validate(sourceUrl: string): void {
    ...
    const localDemoHttp =
      ...
      LOCAL_DEMO_HOSTS.has(host.toLowerCase());
    // 生产默认仍只允许 public HTTPS；显式本地演示开关只放行固定本机地址。
    if (!sourceUrl.startsWith("https://") && !localDemoHttp) {
      throw new Error(
    ...
    if (!localDemoHttp && this.ssrfCheck && this.isPrivateAddress(host)) {
  ...
  async fetch(sourceUrl: string, branch: string, localPath: string): Promise<FetchResult> {
    ...
    // python -m http.server 暴露的是 Git dumb HTTP，不支持 shallow capability；
    // 本地精品仓库体积极小，因此本地演示走完整单分支 clone。公网仍保持浅克隆。
    const cloneOptions: Record<string, string | null> = { "--branch": branch };
    if (!this.isLocalDemoHttp(sourceUrl)) cloneOptions["--depth"] = "1";
  ...
  private isLocalDemoHttp(sourceUrl: string): boolean {
    const host = this.extractHost(sourceUrl).toLowerCase();
    return this.allowLocalHttp && sourceUrl.startsWith("http://") && LOCAL_DEMO_HOSTS.has(host);
  }
}
