 ...
 *
 * 同时支持两种 CodeBuddy 回写格式：
 *   1. 用户点击后的 `<question_answer>` XML；
 *   2. `multi_question_result` JSON tool-result。
 *
 * 特别注意：表单刚展示时 JSON 中会带完整 options，但 `answers` 仍为空。
 * 这种消息不是用户答案，绝不能用 substring 从 options 中误选 Team/Agent/Task。
 */
...
interface MultiQuestionAnswers {
  recognized: true;
  answers: Record<string, string>;
}

function asAnswerText(value: unknown): string | null {
  if (typeof value === "string") return value.trim() || null;
  if (Array.isArray(value)) {
    const text = value
      .map((item) => asAnswerText(item))
  ...
  if (value && typeof value === "object") {
    ...
    for (const key of ["answer", "selected", "selectedOption", "value", "label", "text", "content"]) {
      const text = asAnswerText(record[key]);
      if (text) return text;
    }
  }
  return null;
...
function questionId(question: any, index: number): string {
  for (const value of [question?.id, question?.questionId, question?.key, question?.name]) {
...
function optionTexts(question: any): string[] {
  ...
  return question.options
    .map((option: unknown) => asAnswerText(option))
    .filter((option: string | null): option is string => Boolean(option));
}

/** Resolve clients that return "B"/"2" instead of the selected option label. */
function resolveOptionReference(text: string, question: any): string {
...
function explicitQuestionAnswer(question: any): string | null {
  const direct = asAnswerText(
    question?.answer ?? question?.selected ?? question?.selectedOption ?? question?.value,
  );
  if (direct) return resolveOptionReference(direct, question);

  // CodeBuddy 4.11.x has emitted `questions[].answers` for a selected value in
  // some builds. Older display-only envelopes may put every candidate there,
  // so accept it only when it is unambiguously one selection and not a copy of
  // the complete option list.
  const raw = question?.answers;
  if (raw === undefined || raw === null) return null;
  const options = optionTexts(question);
  if (Array.isArray(raw)) {
    if (raw.length !== 1) return null;
    const selected = asAnswerText(raw[0]);
    if (!selected) return null;
    if (options.length === 1 && selected === options[0]) return null;
    return resolveOptionReference(selected, question);
  }
  if (typeof raw === "object") {
    const selected = asAnswerText(raw);
    return selected ? resolveOptionReference(selected, question) : null;
  }
  return null;
}

/**
 * Parse CodeBuddy's JSON tool result without ever treating question options as answers.
 * `recognized=true, answers={}` means "the form is displayed and still waiting".
 */
function parseMultiQuestionAnswers(content: string): MultiQuestionAnswers | null {
  ...
  try {
    ...
    const result = candidates.find(
      (candidate) => candidate?.type === "multi_question_result" && Array.isArray(candidate.questions),
    );
    ...
    if (result.answers && typeof result.answers === "object" && !Array.isArray(result.answers)) {
      for (const [rawId, raw] of Object.entries(result.answers as Record<string, unknown>)) {
        const text = asAnswerText(raw);
        if (!text) continue;
        const id = rawId.trim().toLowerCase();
        const question = questions.find((item, index) => questionId(item, index) === id);
        answers[id] = question ? resolveOptionReference(text, question) : text;
        orderedAnswers.push(text);
      }
    } else if (Array.isArray(result.answers)) {
      for (const [index, raw] of result.answers.entries()) {
        const entry = raw && typeof raw === "object" ? raw as Record<string, unknown> : null;
        const id = entry
          ? questionId(entry, index)
          : questionId(questions[index], index);
        const text = asAnswerText(entry
          ? entry.answer ?? entry.selected ?? entry.selectedOption ?? entry.value ?? entry.label
          : raw);
        if (!text) continue;
        answers[id] = resolveOptionReference(text, questions[index]);
        orderedAnswers.push(text);
      }
    }

    // Some clients key the top-level answer map by the full question sentence
    // instead of our stable id. Preserve insertion order and map those values
    // back to the displayed questions.
    if (orderedAnswers.length === questions.length) {
      questions.forEach((question, index) => {
        const id = questionId(question, index);
        if (!answers[id]) answers[id] = resolveOptionReference(orderedAnswers[index], question);
      });
    }
    ...
    questions.forEach((question, index) => {
      ...
      const text = explicitQuestionAnswer(question);
      if (text) answers[id] = text;
    });

    return { recognized: true, answers };
  } catch {
    return null;
  }
}

function getJsonAnswer(content: string, ...ids: string[]): string | null | undefined {
  const parsed = parseMultiQuestionAnswers(content);
...
export function extractAssetConfirm(content: string): boolean | null {
  const jsonAnswer = getJsonAnswer(content, "asset_confirm", "q1");
  if (jsonAnswer === null) return null;
  if (jsonAnswer !== undefined) content = jsonAnswer;
...
export function extractTeamFromOptionText(
  ...
  const jsonAnswer = getJsonAnswer(content, "team", "q1");
  // Recognized JSON with no answer is only the form-display acknowledgement.
  if (jsonAnswer === null) return null;
  if (jsonAnswer !== undefined) content = jsonAnswer;
...
export function extractFromOptionText(
  ...
  const json = parseMultiQuestionAnswers(content);
  if (json) {
    const agentAnswer = json.answers.agent ?? json.answers.q1 ?? null;
    const taskAnswer = json.answers.task ?? json.answers.q2 ?? null;
    if (!agentAnswer && !taskAnswer) return null;
    content = [agentAnswer, taskAnswer].filter(Boolean).join("\n");
  }
  ...
  if (xml) {
    ...
    agentText = json.answers.agent ?? json.answers.q1 ?? null;
    taskText = json.answers.task ?? json.answers.q2 ?? null;
  }
  ...
  // The form intentionally hides the Agent question when the selected Team has
  // exactly one Agent. In that case the sole Agent is an explicit system choice,
  // not missing user input (the user only needs to select a Task).
  if (!agentId && team.agents.length === 1) agentId = team.agents[0].agent_id;
  if (!agentId && !xml && !json) agentId = matchAgentInTeam(content, team);
...
export function extractAgentOnly(
  ...
  const opencodeAnswer = extractOpencodeAnswers(content);
  if (opencodeAnswer !== null) content = opencodeAnswer;
  const jsonAnswer = getJsonAnswer(content, "agent", "q1");
  if (jsonAnswer === null) return null;
  if (jsonAnswer !== undefined) content = jsonAnswer;
  const xml = parseQuestionAnswerXml(content);
  if (xml?.agentAnswer) content = xml.agentAnswer;
  const trimmed = content.trim();
...
export function extractTaskOnly(
  ...
  const opencodeAnswer = extractOpencodeAnswers(content);
  if (opencodeAnswer !== null) content = opencodeAnswer;
  const jsonAnswer = getJsonAnswer(content, "task", "q1", "q2");
  if (jsonAnswer === null) return null;
  if (jsonAnswer !== undefined) content = jsonAnswer;
  const xml = parseQuestionAnswerXml(content);
  if (xml?.taskAnswer) content = xml.taskAnswer;
  const trimmed = content.trim();
  if (!trimmed) return null;
