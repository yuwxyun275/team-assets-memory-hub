...
export const AGENT_FORM_TITLE = "会话初始化 — 选择 Agent";
export const TASK_FORM_TITLE = "会话初始化 — 选择任务";
 ...
 * 进入 team / agent_select / task_select 后没有按钮内跳过，需要下一次会话重新选择。
...
export function containsFormTitle(s: string): boolean {
  return (
    ...
    s.includes(AGENT_FORM_TITLE) ||
    s.includes(TASK_FORM_TITLE) ||
 ...
 * CB form 支持的 stage。
 *
 * 所有客户端使用 "asset_confirm" | "team" | "agent_select" | "task_select"。
 * "agent_task" 仅用于读取升级前遗留的会话状态，新会话不再生成组合表单。
 */
...
export interface FormData {
  ...
  /**
   * agent_select 阶段选定的 agent_id，透传给 task_select stage form。
   */
...
function buildFollowupQuestionArgs(data: FormData): { title: string; questions: Array<Record<string, unknown>> | string } {
  ...
  // agent_select 与 task_select 每次只生成一个必答问题；agent_task 仅保留
  // 为旧状态提供兼容渲染，不再由新会话创建。
