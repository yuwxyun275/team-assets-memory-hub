...
export interface AgentContextMetadata {
  /** Server-only, never serialized: acknowledged after final upstream accepts input. */
  assetDelivery?: import("../assets/delivery.js").AssetDelivery;
  protocol: Protocol;
  traceId: string;
  keyId: string;
  modelId: string;
...
export type InjectionPoint =
  | "context.tail"        // New dynamic evidence AFTER the existing conversation; preserve cached prefix
...
export interface InjectionHook {
  ...
  /**
   * Called only after the pipeline has successfully applied the returned
   * blocks to the protocol-neutral context. This is the acknowledgement
   * boundary for audit systems that must distinguish selected from injected.
   * Failures are best-effort and never remove an already applied block.
   */
  onApplied?(ctx: AgentContext, blocks: ContextBlock[]): Promise<void> | void;
}
