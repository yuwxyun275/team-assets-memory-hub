...
export async function resolveFixedAssetCtxs(
  ...
  try {
    ...
    for (const item of detail.items) {
      // A reviewed memory unit never grants access to the source agent's entire mutable conversation store.
      // Shared reviewed content is injected by the version-bound team-assets path instead.
