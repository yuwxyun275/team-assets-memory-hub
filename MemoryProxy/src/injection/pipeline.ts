...
import type { AssetHistoryCoordinator, HistoryHooks } from "../assets/history.js";
import {benchmarkBinding,moveBenchmarkAugmentsToFront,auditBenchmarkRequest,validateBenchmarkRequest} from "../assets/benchmark.js";
import {deliveredContext} from "../assets/delivery.js";

/** Optional pipeline behaviors (agent detection, etc.). */
export interface InjectionPipelineOptions {
  assetHistory?: AssetHistoryCoordinator;
  /**
...
export class InjectionPipeline {
  ...
  private assetHistory?: AssetHistoryCoordinator;
  ...
  constructor(
    ...
    this.assetHistory = options.assetHistory;
  }
  ...
  async process(
    ...
    try {
      ...
      await this.assetHistory?.prepare(ctx);
      const benchmark=benchmarkBinding(metadata.custom?.session,metadata.userId);
      if(benchmark)ctx.metadata.custom={...ctx.metadata.custom,assetBenchmark:benchmark};
      if(benchmark)validateBenchmarkRequest(ctx,benchmark);
      ...
      const hookResults: HookResult[] = benchmark?.arm === "no_team_assets" ? [] : this.assetHistory
        ? await this.assetHistory.process(ctx, () => this.executeHooks(ctx))
        : await this.executeHooks(ctx);

      // 4. Serialize → modified body
      if(benchmark){
        if(benchmark.arm==="front_dynamic_cards")moveBenchmarkAugmentsToFront(ctx);
        auditBenchmarkRequest(ctx,benchmark);
      }
      ...
      metadata.assetDelivery?.discard();
  ...
  private async executeHooks(ctx: AgentContext): Promise<HookResult[]> {
    ...
    for (const point of executionOrder) {
      ...
      for (const hook of hooks) {
        if(ctx.metadata.custom?.assetBenchmark && hook.id!=="team-assets-orchestrator-injector")continue;
        const hookStartMs = Date.now();
        // ── Observer: hook start ──────────────────────────────────────────
        safeCall(() => this.observer.onHookStart(hook, point));

        try {
          let blocks = await this.resolveHookBlocks(hook, ctx, spaceId, userId, agentSource, sessionId);
          const history = hook.id === "team-assets-orchestrator-injector" && point === "context.tail"
            ? ctx.metadata.custom?.assetHistoryHooks as HistoryHooks | undefined : undefined;
          if (history) blocks = history.filter(blocks);
          ...
          if (blocks.length > 0) {
            ...
            history?.capture(blocks);
            if (hook.onApplied) {
              try {
                if (hook.id === 'team-assets-orchestrator-injector' && ctx.metadata.assetDelivery) {
                  ctx.metadata.assetDelivery.defer(async texts => { await hook.onApplied!(deliveredContext(ctx,texts),blocks); });
                } else await hook.onApplied(ctx, blocks);
    } catch (err) {
                console.warn(
                  `[injection] post-apply acknowledgement failed for hook "${hook.id}":`,
          ...
          if(ctx.metadata.custom?.assetBenchmark)throw error;
  ...
  private applyInjection(
    ...
    this.applyByPoint(ctx, point, blocks, hook.id);
  }
  ...
  private applyByPoint(
    ...
    switch (point) {
      ...
      case "context.tail": {
        const text = blocks.filter(block => block.type === "text").map(block => block.content).join("\n\n");
        if (text) ctx.messages.push({ role: "user", blocks: [{ type: "text", content: text }], metadata: hookId === "team-assets-orchestrator-injector" ? { proxyAssetAugment: true } : undefined });
        break;
      }
