...
export { TeamAssetsOrchestratorInjector } from "./injectors/team-assets-orchestrator-injector.js";
...
import { TeamAssetsOrchestratorInjector } from "./injectors/team-assets-orchestrator-injector.js";
...
import { AssetHistoryCoordinator } from "../assets/history.js";
import { configuredHistoryStore } from "../assets/history-store.js";
...
function buildPipelineBundle(config: ProxyConfig): PipelineBundle {
  ...
  if (injectors.includes("team-assets") && config.injection.teamAssets.enabled) {
    registry.register(new TeamAssetsOrchestratorInjector({ ...config.injection.teamAssets,
      bridgeBaseUrl: config.injection.externalGatewayUrl || `http://127.0.0.1:${config.server.port}`,
    }, config.coreSkill));
  }
  ...
  if (config.injection?.assetReflection?.markerOptIn) {
    ...
    if (registeredIds.has("team-assets-orchestrator-injector")) {
      activeAssetTags.push("team_assets");
    }
  ...
  const historyStore = injectors.includes("team-assets") && config.injection.teamAssets.enabled
    ? configuredHistoryStore(config) : undefined;
  const pipeline = new InjectionPipeline(registry, adapters, {
    assetHistory: historyStore ? new AssetHistoryCoordinator({ store: historyStore, core: config.coreSkill,
      bridgeBaseUrl: config.injection.externalGatewayUrl || `http://127.0.0.1:${config.server.port}`,
      tokenBudget: config.injection.teamAssets.historyTokenBudget ?? 12000,
      pinnedAssetIds: config.injection.teamAssets.historyPinnedAssetIds,
      loadTurnFloor: async session => {
        const settings = config.injection.teamAssets;
        const response = await fetch(`${settings.endpoint.replace(/\/$/, '')}/v2/turns?session_id=${encodeURIComponent(session.session_id)}`, {
          headers: settings.serviceToken ? {authorization: `Bearer ${settings.serviceToken}`} : {},
          signal: AbortSignal.timeout(settings.timeoutMs || 10000),
        });
        if (!response.ok) throw new Error('asset_turn_cursor_unavailable');
        const result = await response.json() as any;
        if (!Array.isArray(result.turns)) throw new Error('invalid_asset_turn_cursor');
        return Math.max(0, ...result.turns.filter((t:any) => t.session_id === session.session_id && t.team_id === session.team_id
          && t.task_id === session.task_id && t.agent_id === session.agent_id).map((t:any) => Number(t.turn_seq) || 0));
      },
    }) : undefined,
    hookCacheRepo,
