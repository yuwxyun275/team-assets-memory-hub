...
export async function handleCodexEndpoint(
  ...
  if (!injectionSkipped && sessionInfo && config.injection?.enabled && (config.injection.injectors?.length ?? 0) > 0) {
    try {
      ...
      const evidenceMessages = codexInputAsOpenAiMessages(input);
  ...
  return forwardToUpstream(c, config, body, traceId, startTime, keyId, modelId, pipe, lf, archiveCtx, assetDelivery);
}
...
async function forwardToUpstream(
  ...
  try {
    ...
    await assetDelivery?.accept(body, upstreamResp.status);
  } catch (err: unknown) {
    assetDelivery?.discard();
...
/** Convert Responses input history into the small Chat Completions subset used
 * by the shared injection/evidence pipeline. Encrypted reasoning is omitted. */
export function codexInputAsOpenAiMessages(input: unknown): Array<Record<string, unknown>> {
