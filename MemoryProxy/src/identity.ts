...
function extractUserInfoFromBody(body: Record<string, unknown>): UserInfoFromPrompt | null {
  ...
  const contentText = (content: unknown): string => {
    ...
    return (content as Array<{ type?: string; text?: string; content?: string }>)
      .filter((item) => item?.type === "text" || typeof item?.text === "string")
      .map((item) => item.text ?? item.content ?? "")
      .join("\n");
  };

  // CodeBuddy 4.x places <user_info> in the first user message, while older
  // clients used a system message. Anthropic-style clients may use body.system.
  // Search all three documented locations and prefer a block that actually
  // contains <user_info> instead of assuming one wire layout.
  ...
  if (Array.isArray(messages)) {
    ...
    for (const message of [...userMessages].reverse().concat([...systemMessages].reverse())) {
      const raw = contentText((message as Record<string, unknown>).content);
      candidates.push(codebuddySummary(raw)?.current ?? raw);
    }
  }
  candidates.push(contentText(body.system));
  const systemContent = candidates.find((item) => /<user_info>[\s\S]*?<\/user_info>/.test(item))
