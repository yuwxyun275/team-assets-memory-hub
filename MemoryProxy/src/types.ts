...
export interface InjectionConfig {
  ...
  /** Evidence-first multi-source team asset orchestrator (disabled by default). */
  teamAssets: {
    enabled: boolean;
    /** Reviewed assets: lightweight cards + explicit versioned read through asset-bridge. */
    progressiveDisclosure?: boolean;
    /** Durable original-position replay and compaction checkpoints (single-host). */
    historyEnabled?: boolean;
    historyDirectory?: string;
    /** Total Proxy-owned historical payload budget, separate from each recall. */
    historyTokenBudget?: number;
    historyPinnedAssetIds?: string[];
    inlineMaxChars?: number;
    readMaxChars?: number;
    /** Internal orchestrator URL used by MemoryProxy. */
    endpoint: string;
    /** URL exposed to the coding agent for structured used/validated events. */
    externalUrl: string;
    /** Optional service bearer; never forwarded to the upstream model. */
    serviceToken: string;
    timeoutMs: number;
    /** Admin safety ceilings; the task profiler normally chooses smaller runtime values. */
    tokenBudget: number;
    maxAssets: number;
    /** Fallback hints only; Memory Hub task metadata and automatic profiling take precedence. */
    repository: string;
    version: string;
    taskType: string;
    targetPaths: string[];
