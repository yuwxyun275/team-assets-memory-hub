 ...
 */
import { codebuddySummary } from "./codebuddy-summary.js";
...
export function extractUserQueryText(raw: string): string {
  // Historical <user_query> inside cb_summary must never become today's query.
  raw = codebuddySummary(raw)?.current ?? raw;
