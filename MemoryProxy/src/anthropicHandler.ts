...
async function forwardWithRetry(
  ...
  if (shouldRetry && target.retryTarget) {
    ...
    try {
      ...
      return { resp: upstreamResp, retried: true, sentBody: originalBody };
  ...
  if (!injectedSkipped && !skipInjection && config.injection?.enabled && config.injection.injectors.length > 0) {
    try {
      ...
      const injectedBody = await pipeline.process(body, {
        ...
        custom: sessionInfo ? { session: sessionInfo, taskDetail: cachedTaskDetail, userKey: callerUserKey ?? undefined, assetCapabilities, workspaceFolder: teamAssetCli?.workspace } : undefined,
