...
export class MetadataClient {
  async quality<T = any>(action: string, body: Record<string, unknown>): Promise<T> {
    return this.fetch<T>(`/v3/meta/asset/quality/${action}`, body);
  }
   ...
   * Owner filtering is reserved for explicit personal/administrative views.
   * Session initialization omits it after Team membership has been verified,
   * because Agents attached to a Team are shared execution roles.
   *
  ...
  private async fetch<T>(
    ...
    if (!resp.ok) {
      ...
      console.log(
        `[metadata-client] path=${path} status=${resp.status} serviceId=${this.serviceId}`,
      );
