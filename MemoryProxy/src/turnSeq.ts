...
function isHumanUserContent(content: unknown): boolean {
  if (typeof content === "string") {
    return !isInfrastructureMessage(content);
  }
  if (Array.isArray(content)) {
    for (const block of content) {
      ...
      if (b && typeof b === "object" && b.type === "text") {
        ...
        if (!isInfrastructureMessage(text)) return true;
      }
    }
    return false;
  }
...
/** Session-initialization form answers are control-plane traffic, not coding turns. */
function isInfrastructureMessage(text: string): boolean {
  const value = text.trim();
  return value.startsWith("<system-reminder>")
    || value.startsWith("<question_answer>")
