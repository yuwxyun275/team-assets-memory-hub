import { cliBinding, cliBindingMatches } from "./session/codebuddy/cli-binding.js";
/** Core request handler: intercept → forward → parse usage → log. */

import type { Context } from "hono";
import { AssetDelivery } from "./assets/delivery.js";
import {benchmarkBinding,BENCH_NO_NATIVE_ASSETS,auditBenchmarkResponse} from "./assets/benchmark.js";
...
import { observeTeamAssetAssistantResponse } from "./injection/injectors/team-assets-response-observer.js";
import {
  stripTeamAssetControlMarkup,
  TeamAssetControlMarkupFilter,
} from "./injection/injectors/team-assets-control-markup.js";
import {
  enforceRateLimit,
...
const SKIP_REQUEST_HEADERS = new Set([
  "x-team-assets-cli", "x-team-assets-workspace", "x-team-id", "x-agent-id", "x-task-id",
  "host",
  "content-length",
  "transfer-encoding",
  "connection",
...
async function forwardWithRetry(
...
): Promise<{ resp: Response; retried: boolean; sentBody: Record<string, unknown> }> {
  ...
  if (shouldRetry && target.retryTarget) {
    ...
    try {
      ...
      return { resp: upstreamResp, retried: true, sentBody: retryBody };
  ...
  return { resp: upstreamResp, retried: false, sentBody: upstreamBody };
}
...
export async function handleChatCompletions(
  ...
  const inspectedIdentity = inspectAndRecord(
    "POST",
    c.req.path,
    reqHeaders,
      ...
      if (memCmd && isMemCommandAllowed(config.memCommand, memCmd.command)) {
        ...
        if (oldState?.status === "initialized" && oldState.sessionInfo && config.coreSkill?.endpoint) {
          ...
          if (si.space_id && si.user_id && si.team_id && si.agent_id) {
            const archiveSpaceId = si.space_id;
  ...
  let cachedTaskDetail: import("./session/types.js").TaskDetail | null = null;
  ...
  let teamAssetCli: ReturnType<typeof cliBinding>;
  try { teamAssetCli = cliBinding(lcHeaders, config.sessionInit); }
  catch { return c.json({ error: "cli_binding_invalid_or_disabled" }, 400); }
  if (config.sessionInit?.enabled && conversationId && !isAuxiliary && !_dshHeadless) {
    try {
      ...
      if (teamAssetCli && (initResult.intercepted || initResult.bypassed || !cliBindingMatches(teamAssetCli, initResult.sessionInfo))) return c.json({ error: "cli_binding_not_authorized_or_session_mismatch" }, 403);
      ...
      // CodeBuddy 交互式首次绑定完成后必须停靠一轮。否则最初用于唤起表单的
      // “你好”等消息会在同一个 HTTP 请求中携带完整 Task 上下文发给模型，模型
      // 会误把它当成“立即执行”。请求头预选身份不带 awaitingUserInstruction，
      // 因此自动化/无 UI 客户端仍可在首轮直接执行真实指令。
      const {
        shouldPauseAfterInteractiveRegistration,
        buildRegistrationConfirmationText,
      } = await import("./session/codebuddy/registration-confirmation.js");
      if (shouldPauseAfterInteractiveRegistration(agentSource, initResult)) {
        const { buildMemResponse } = await import("./mem-command/response-builder.js");
        const text = buildRegistrationConfirmationText(initResult);
        console.log(
          `[session-init:cb] interactive registration complete; ` +
          `acknowledging locally without retrieval/injection/upstream session=${sessionKey}`,
        );
        return buildMemResponse(text, {
          protocol: "openai",
          stream: isStream,
          requestId: `session-init-complete-${Date.now()}`,
        });
      }
      ...
      if(benchmarkBinding(initResult.sessionInfo,userId||undefined))assetCapabilities={...BENCH_NO_NATIVE_ASSETS};
      ...
      if (initResult.resetFlow && initResult.justRegistered && !initResult.bypassed) {
        _resetFlowResult = {
          ...
          agentIdShort: initResult.sessionInfo?.agent_id?.slice(-8) ?? "",
          teamId: initResult.sessionInfo?.team_id?.slice(-8) ?? "",
  ...
  if (teamAssetCli && (injectedSkipped || !cliBindingMatches(teamAssetCli, sessionInfo))) {
    return c.json({ error: "cli_binding_unavailable" }, 403);
  }
  if (_resetFlowResult) {
  ...
  // The durable asset cursor replaces this fallback after session resolution.
  // Carry the same resolved identity through injection and response evidence.
  let teamAssetTurnSeq = countHumanTurns(messages, "openai");
  const assetDelivery = new AssetDelivery();
      ...
      const injectedBody = await pipeline.process(body, {
        ...
        custom: sessionInfo
          ? {
              ...
              workspaceFolder: teamAssetCli?.workspace ?? inspectedIdentity.userInfo?.workspaceFolder ?? undefined,
            }
          : undefined,
      });
      body = injectedBody;
      teamAssetTurnSeq = assetDelivery.turnSeq ?? teamAssetTurnSeq;
      messages = Array.isArray(injectedBody.messages) ? injectedBody.messages : messages;
    } catch (err: unknown) {
      assetDelivery.discard();
      // Injection failure is non-fatal — fall back to original body
      if(benchmarkBinding(sessionInfo,userId||undefined))return new Response(JSON.stringify({error:err instanceof Error&&err.message==='workspace_confirmation_required'
        ? 'workspace_confirmation_required; 请在当前消息首行输入“确认当前工作区：绝对路径”，再继续任务。no upstream request sent'
        : "benchmark_injection_failed; no upstream request sent"}),{status:503,headers:{"content-type":"application/json"}});
    }
  }
  ...
  try {
    ...
    await assetDelivery.accept(result.sentBody, upstreamResp.status);
  } catch (err: unknown) {
    assetDelivery.discard();
  ...
  if (isStream) {
    ...
    const tapCtx: TapContext = {
      qualityUserKey: apiKey || config.tdai?.apiKey || "",
      config,
  ...
  auditBenchmarkResponse(sessionInfo,userId||undefined,traceId,usage,assistantMessage);
  ...
  await observeTeamAssetAssistantResponse(config, {
    qualityContext: { spaceId: spaceId ?? "", userKey: apiKey || config.tdai?.apiKey || "", teamId: String(sessionInfo?.team_id ?? ""), taskId: String(sessionInfo?.task_id ?? "") },
    sessionId: String(sessionInfo?.session_id ?? sessionKey),
    turnSeq: teamAssetTurnSeq,
    actorId: String(sessionInfo?.agent_id ?? ""),
    text: typeof assistantMessage?.content === "string" ? assistantMessage.content : "",
    toolCalls: (Array.isArray(assistantMessage?.tool_calls) ? assistantMessage.tool_calls : [])
      .map((tc) => {
  ...
  return new Response(sanitizeOpenAiResponseForClient(respText), {
    status: upstreamResp.status,
    headers: respHeaders,
  });
}
...
interface TapContext {
  ...
  /** Human-turn identity shared with the team-asset injection request. */
  teamAssetTurnSeq: number;
...
/** Remove proxy/model evidence declarations from the user-visible JSON body. */
function sanitizeOpenAiResponseForClient(text: string): string {
  try {
    ...
    for (const choice of choices) {
      ...
      if (typeof record.content === "string") {
        const sanitized = stripTeamAssetControlMarkup(record.content);
        changed ||= sanitized !== record.content;
...
/** Rewrite only content deltas. Usage and tool-call deltas remain untouched. */
function sanitizeOpenAiSseEventForClient(
  event: string,
  filter: TeamAssetControlMarkupFilter,
): string {
  return event.split("\n").map((line) => {
    ...
    try {
      ...
      for (const choice of choices) {
        ...
        if (typeof record.content === "string") {
          const sanitized = filter.push(record.content);
          changed ||= sanitized !== record.content;
          record.content = sanitized;
        }
      }
      return changed ? `${match[1]}${JSON.stringify(value)}` : line;
    } catch {
      return line;
    }
  }).join("\n");
...
/** Create a TransformStream that extracts usage/content/tool_calls from SSE
 *  events and removes internal evidence declarations from the client stream.
 */
function createUsageTapTransform(ctx: TapContext): TransformStream<Uint8Array, Uint8Array> {
  ...
  const clientMarkupFilter = new TeamAssetControlMarkupFilter();
  let sseBuf = "";
  let lastUsage: Record<string, unknown> | null = null;
  ...
  async function finalize(): Promise<void> {
    ...
    auditBenchmarkResponse(ctx.sessionInfo,String(ctx.sessionInfo?.user_id||""),traceId,lastUsage,outputMessage);
    await observeTeamAssetAssistantResponse(ctx.config, {
      qualityContext: { spaceId: ctx.spaceId ?? "", userKey: ctx.qualityUserKey ?? "", teamId: String(ctx.sessionInfo?.team_id ?? ""), taskId: String(ctx.sessionInfo?.task_id ?? "") },
      sessionId: String(ctx.sessionInfo?.session_id ?? sessionKey),
      turnSeq: ctx.teamAssetTurnSeq,
      actorId: String(ctx.sessionInfo?.agent_id ?? ""),
      text: assistantContent,
  ...
  return new TransformStream<Uint8Array, Uint8Array>({
    transform(chunk, controller) {
      try {
        for (const event of processSseChunk(decoder.decode(chunk, { stream: true }))) {
          controller.enqueue(encoder.encode(`${sanitizeOpenAiSseEventForClient(event, clientMarkupFilter)}\n\n`));
        }
      } catch (err: unknown) {
        pipe.error("STREAM_TAP", err);
        // Fail open on malformed SSE: the upstream response is still usable.
    ...
    async flush(controller) {
      try {
        ...
        for (const event of processSseChunk(tail)) {
          controller.enqueue(encoder.encode(`${sanitizeOpenAiSseEventForClient(event, clientMarkupFilter)}\n\n`));
        }
        if (sseBuf) {
          const finalEvent = sseBuf;
          const usage = extractSseUsage(finalEvent);
          if (usage) lastUsage = usage;
          const { content, toolCallDeltas } = extractSseContentAndTools(finalEvent);
          assistantContent += content;
          mergeToolCallDeltas(toolCallAccumulators, toolCallDeltas);
          controller.enqueue(encoder.encode(sanitizeOpenAiSseEventForClient(finalEvent, clientMarkupFilter)));
          sseBuf = "";
        }
        // An unterminated machine declaration is intentionally discarded. An
        // ordinary suffix is practically never buffered beyond the final SSE
        // event, but flushing clears the state deterministically.
        clientMarkupFilter.flush();
        await finalize();
      } catch (err: unknown) {
