...
import {benchmarkBinding} from "../assets/benchmark.js";
...
export function createMemoryBridgeHandler(
  ...
  return async (c: Context): Promise<Response> => {
    ...
    if(benchmarkBinding(ids,ids.user_id))return envelope(40301,"benchmark_native_asset_side_channel_disabled",403);
      ...
      if (raw && raw.trim()) {
        ...
        if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
          ...
          emitBridgeRejectTelemetry({
            ...
            agentId: ids.agent_id, agentSource: ids.composite_key?.includes(':') ? ids.composite_key.split(':')[0] : undefined,
          });
