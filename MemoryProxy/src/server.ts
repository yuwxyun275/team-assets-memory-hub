...
import { createAssetBridgeHandler } from "./assets/asset-bridge.js";
...
export function createApp(config: ProxyConfig): Hono {
  ...
  if (config.injection?.teamAssets?.progressiveDisclosure) ensureBindingRepoPersistent(config);
...
// Skill bridge: LLM curls land here, proxy injects auth + identity, forwards to core.
  ...
  const assetBridgeHandler = createAssetBridgeHandler(config);
  app.all("/asset-bridge/*", c => assetBridgeHandler(c));
