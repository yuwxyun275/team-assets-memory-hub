...
export async function initProxyStorage(config: StorageConfig): Promise<ProxyStorage> {
  ...
  if (config.backend === "cos") {
    try {
      // Optional private extension: OSS typechecking must not require it.
      // Runtime export validation and the existing fallback remain unchanged.
      const optionalModule: string = "@context-proxy/cost-guard";
