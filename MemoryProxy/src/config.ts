...
export function buildConfig(overrides: CliOverrides = {}): ProxyConfig {
  ...
  return {
    ...
    injection: {
      ...
      teamAssets: {
        enabled: yaml.injection?.teamAssets?.enabled ?? DEFAULT_CONFIG.injection.teamAssets.enabled,
        progressiveDisclosure: yaml.injection?.teamAssets?.progressiveDisclosure ?? DEFAULT_CONFIG.injection.teamAssets.progressiveDisclosure,
        historyEnabled: yaml.injection?.teamAssets?.historyEnabled ?? true,
        historyDirectory: yaml.injection?.teamAssets?.historyDirectory ?? "",
        historyTokenBudget: Math.max(1000, Math.min(64000, Number(yaml.injection?.teamAssets?.historyTokenBudget ?? 12000) || 12000)),
        historyPinnedAssetIds: Array.isArray(yaml.injection?.teamAssets?.historyPinnedAssetIds)
          ? yaml.injection.teamAssets.historyPinnedAssetIds.filter(v => typeof v === "string" && v.length <= 200).slice(0, 100) : [],
        inlineMaxChars: Math.max(0, Math.min(4000, Number(yaml.injection?.teamAssets?.inlineMaxChars ?? 600) || 0)),
        readMaxChars: Math.max(1, Math.min(60_000, Number(yaml.injection?.teamAssets?.readMaxChars ?? 60_000) || 60_000)),
        endpoint: yaml.injection?.teamAssets?.endpoint ?? DEFAULT_CONFIG.injection.teamAssets.endpoint,
        externalUrl: yaml.injection?.teamAssets?.externalUrl ?? DEFAULT_CONFIG.injection.teamAssets.externalUrl,
        serviceToken: expandEnv(
          yaml.injection?.teamAssets?.serviceToken ?? DEFAULT_CONFIG.injection.teamAssets.serviceToken,
        ),
        timeoutMs: yaml.injection?.teamAssets?.timeoutMs ?? DEFAULT_CONFIG.injection.teamAssets.timeoutMs,
        tokenBudget: yaml.injection?.teamAssets?.tokenBudget ?? DEFAULT_CONFIG.injection.teamAssets.tokenBudget,
        maxAssets: yaml.injection?.teamAssets?.maxAssets ?? DEFAULT_CONFIG.injection.teamAssets.maxAssets,
        repository: yaml.injection?.teamAssets?.repository ?? DEFAULT_CONFIG.injection.teamAssets.repository,
        version: yaml.injection?.teamAssets?.version ?? DEFAULT_CONFIG.injection.teamAssets.version,
        taskType: yaml.injection?.teamAssets?.taskType ?? DEFAULT_CONFIG.injection.teamAssets.taskType,
        targetPaths: yaml.injection?.teamAssets?.targetPaths ?? DEFAULT_CONFIG.injection.teamAssets.targetPaths,
      },
    },
    extraction: {
